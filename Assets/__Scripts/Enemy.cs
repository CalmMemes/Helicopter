...
public class Enemy : MonoBehaviour
{
    ...
    public float showDamageDuration = 0.1f; //Длительность эффекта попадания в секунду
    ...
    public float damageDoneTime; //Время пекращения отображения эффекта 
    public bool notifiedOfDestruсtion = false;
        ...
        switch (otherGO.tag)
        {
            case "ProjectileHero":
                ...
                if (health <= 0)
                {
                    ...
                    if (!notifiedOfDestruсtion)
                    {
                        Main.S.ShipDestroyed(this);
                    }
                    notifiedOfDestruсtion = true;
                    //Уничтожить этот вражеский корабль
                    Destroy(this.gameObject);
                    Hit.hitsCounter += score;
                }
                Destroy(otherGO);
                break;
